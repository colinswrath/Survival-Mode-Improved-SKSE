...
public:
	...
	//TODO-Temporary
	static void ForEachReferenceInRange(RE::TESObjectREFR* a_origin, float a_radius, std::function<RE::BSContainer::ForEachResult(RE::TESObjectREFR& a_ref)> a_callback)
	{
		if (a_origin && a_radius > 0.0f) {
			...
			if (interiorCell) {
				interiorCell->ForEachReferenceInRange(originPos, a_radius, [&](RE::TESObjectREFR& a_ref) {
